...
router.get('/home', (req, res) => {
    ...
    Slot.find({})
        ...
        .exec((err, slots) => {
            if (err)
                return console.log(
                    `MESSAGE: Error querying slots ERROR: ${err}`,
                );
            ...
            Slot.countDocuments({ state: 'vacant' }, (err, count) => {
                if (err)
                    return console.log(
                        `MESSAGE: Error in COUNTING slots ERROR: ${err}`,
                    );
                ...
                User.find({})
                    ...
                    .then(users => {
                        if (err)
                            return console.log(
                                `MESSAGE: Error in getting all users ERROR: ${err}`,
                            );
                        ...
                        users.forEach(user => {
                            if (user.reservation.expiration.raw != undefined) {
                                let expiration = getTime(
                                    new Date(
                                        user.reservation.expiration.raw,
                                    ).getTime(),
                                );
