...
router.post('/', (req, res) => {
    ...
    Slot.findOne({ slotLetter: slotLetter }, (err, slot) => {
        if (err)
            return console.log(`MESSAGE: Error finding the slot ERROR: ${err}`);

        // find the user who is reserving in the DB
        User.findOne({ username: username }, (err, user) => {
            if (err)
                return console.log(
                    `MESSAGE: Error in finding user ERROR: ${err}`,
                );
            ...
            if (
                ...
                if (requestFrom === 'user') {
                    ...
                    return res.json({
                        response: `${username}'s balance is not enough`,
                    });
                }
            }
            ...
            if (user.reservation.slot != undefined) {
                if (requestFrom === 'user') {
                    return res.json({
                        response:
                            "can't reserve because you have an active subscription",
                    });
                } else {
                    return res.json({
                        response: `can't reserve because ${username} have an active subscription`,
                    });
                }
            }
            ...
            // set current balance as previous before deducting
            // for logging
            ...
            createLog({
                ...
                parkingFee: slot.slotRate.reservationRate,
                prevBal: prevBal,
                currBal: user.balance,
                expiryTime: formattedEndTime.time,
                slot: slotLetter,
            });
            ...
            if (requestFrom === 'user') {
                res.json({
                    response: `successfully reserved at slot ${slotLetter}`,
                    refresh: true,
                });
            } else {
                res.json({
                    response: `${
                        user.username
                    } has been reserved successfully at slot ${
                        user.reservation.slot
                    }`,
                    refresh: true,
                });
            }
